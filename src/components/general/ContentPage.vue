...
<template>
  <div class="row text-left justify-content-center">
    <div class="col-12 col-md-10 col-xl-8">
      <h1 class="my-4">
        My content<img src="@/assets/images/icons/menu/content.png" alt="" />
      </h1>
      <div class="my-4">
        <div class="sources-intro w-100">
          <span>
            You have {{ sources.length || "no" }} content
            {{ sources.length == 1 ? "source" : "sources" }}.
          </span>
          <span class="no-shrink pb-3 pr-3">
            <strong class="source-amounts">
              <img
                src="@/assets/images/icons/menu/spells.png"
                style="filter: brightness(2.3)"
                title="Total amount of spells"
                alt=""
              />
              {{ app.spells.length }}
              <img
                src="@/assets/images/icons/menu/rules.png"
                class="ml-3"
                style="filter: brightness(2.0)"
                title="Total amount of rules"
                alt=""
              />
              {{ app.rules.length }}
            </strong>
        ...
        <div v-for="source in sources" :key="source.name" class="source mb-3">
          <img
            src="@/assets/images/icons/misc/content.png"
            class="source-icon"
            alt=""
          />
          <div class="source-info pl-2">
            <div class="source-head">
              <span class="source-title d-flex">
                <template v-if="sourceBeingEdited == source">
                  ...
                  <input
                    type="number"
                    class="form-control ml-2 d-none d-md-block"
                    v-model.number="source.version"
                    min="0"
                    title="Version number for this source"
                    style="width:70px"
                  />
                </template>
                <template v-else
                  >{{ source.name }}
                  <span
                    v-if="source.version > 0"
                    class="source-version badge ml-2"
                    >v{{ source.version }}</span
                  ></template
                >
              </span>
              <span class="source-amounts no-shrink ml-4">
                <img
                  ...
                  title="Amount of spells in this source"
                  alt=""
                />
                ...
                <img
                  ...
                  title="Amount of rules in this source"
                  alt=""
                />
                {{ source.rules.length }}
                <img
                  src="@/assets/images/icons/misc/exportFile.png"
                  class="source-action ml-3"
                  @click="exportSource(source)"
                  title="Export this source"
                  alt=""
                />
                <img
                  src="@/assets/images/icons/misc/edition.png"
                  :class="{
                    'source-action ml-2': true,
                    active: sourceBeingEdited == source,
                  }"
                  @click="toggleSourceBeingEdited(source)"
                  title="Edit this source"
                  alt=""
                />
                <img
                  src="@/assets/images/icons/misc/close.png"
                  class="source-action ml-2"
            ...
            <textarea
              v-if="sourceBeingEdited == source"
              class="form-control"
              v-model="source.description"
              maxlength="512"
            ></textarea>
            <div v-else style="line-break: anywhere">
              {{ source.description }}
            </div>
          </div>
        </div>
      ...
      <div class="row sources-actions">
        ...
        <div class="col-12 col-md-6 col-xl-4 mt-1">
          <button
            class="btn btn-primary w-100"
            @click="loadSRD"
            :disabled="sources.some((s) => s.name == 'SRD 5.1')"
          >
        ...
        <div class="col-12 col-md-6 col-xl-4 mt-1">
          <button
            ...
            :disabled="sources.length == 0"
            title="Check your content sources for any issues"
          >
        ...
        <div class="col-12 col-md-6 col-xl-4 mt-1">
          <a
            class="btn btn-primary w-100"
            href="https://github.com/bgior/weavelore/wiki/Content-FAQ"
            target="_blank"
          >
...
export default {
  ...
  methods: {
    ...
    exportDatabase() {
      this.download(
        `WeaveLoreExport-${this.app.contentDatabase.data.sources.length}c-${this.app.spells.length}s-${this.app.rules.length}r.json`,
        this.app.contentDatabase.export(),
      );
    },
    download(name, text) {
    ...
    resetDatabase() {
      if (
        confirm("Are you sure you want to delete all your content sources?")
      ) {
    ...
    deleteSource(source) {
      if (
        source.name == "SRD 5.1" ||
        (source.spells.length == 0 && source.rules.length == 0) ||
    ...
    exportSource(source) {
      this.download(
        source.name + ".json",
        this.app.contentDatabase.export(source.name),
      );
    },
    saveDatabase() {
      try {
    ...
    showError(error) {
      ...
      if (error.name == "ValidationError") {
        ...
        message =
          "Sorry, an unexpected error occurred. See the console for details.";
    ...
    meta: [
      {
        vmid: "description",
        name: "description",
        content: "Manage your WeaveLore content sources from this page.",
      },
