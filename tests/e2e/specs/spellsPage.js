...
describe("Spells page", () => {
  ...
  it("creates, edits, saves and removes a spell correctly", () => {
    ...
    cy.get(
      ".spell-editor > :nth-child(1) > :nth-child(1) > .form-control",
    ).type("{selectall}{backspace}" + spell.name);
    cy.get(".spell-editor > :nth-child(1) > :nth-child(2) > .cs")
      .click()
      .contains(spell.school)
      .click();
    cy.get(".spell-editor > :nth-child(1) > :nth-child(3) > .cs")
      .click()
      .contains(spell.level)
      .click();
    cy.get(
      ".spell-editor > :nth-child(2) > :nth-child(1) > :nth-child(1) > .form-control",
    ).type(spell.range);
    cy.get(":nth-child(1) > :nth-child(2) > .form-control").type(
      spell.duration,
    );
    cy.get(".mb-2 > .form-control").type(spell.casting);
    cy.get(":nth-child(2) > :nth-child(3) > .cs")
      .click()
      .contains(spell.material)
      .click();
    cy.get(":nth-child(2) > :nth-child(3) > :nth-child(2)").click(); // Verbal
    cy.get(
      ".spell-editor > :nth-child(2) > :nth-child(2) > :nth-child(3)",
    ).click(); // Scalable
    cy.get(".pl-4 > .row > :nth-child(9)").click(); // Wizard
    ...
    cy.get(".spell:visible")
      .should("have.length", 1)
      .contains("Expelliarmus")
      .click();
    cy.get(":nth-child(4) > span > .details-icon").click(); // Toggle edition mode on
    ...
    cy.get(
      ".spell-editor > :nth-child(1) > :nth-child(1) > .form-control",
    ).should("have.value", spell.name);
    cy.get(
      ".spell-editor > :nth-child(1) > :nth-child(2) > .cs > .cs-field",
    ).should("contain", spell.school);
    cy.get(
      ".spell-editor > :nth-child(1) > :nth-child(3) > .cs > .cs-field",
    ).should("contain", spell.level);
    cy.get(
      ".spell-editor > :nth-child(2) > :nth-child(1) > :nth-child(1) > .form-control",
    ).should("have.value", spell.range);
    cy.get(":nth-child(1) > :nth-child(2) > .form-control").should(
      "have.value",
      spell.duration,
    );
    cy.get(".mb-2 > .form-control").should("have.value", spell.casting);
    cy.get(":nth-child(2) > :nth-child(3) > .cs > .cs-field").should(
      "contain",
      spell.material,
    );
    cy.get(":nth-child(2) > :nth-child(3) > :nth-child(2)").should(
      "not.have.class",
      "absent",
    ); // Verbal
    cy.get(
      ".spell-editor > :nth-child(2) > :nth-child(2) > :nth-child(3)",
    ).should("not.have.class", "absent"); // Scalable
    cy.get(".pl-4 > .row > :nth-child(9)").should("not.have.class", "disabled"); // Wizard
    cy.get('[rows="4"]').should("have.value", spell.description);
    cy.get('[rows="2"]').should("have.value", spell.atHigherLevel);
    cy.get(":nth-child(7) > .form-control").should(
      "have.value",
      spell.materials,
    );
